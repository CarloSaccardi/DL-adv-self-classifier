...
import glob
import wget 
sys.path.append(os.path.join(os.path.dirname(__file__), "saliency_investigation"))
...
if __name__ == '__main__':
    url_fix = "https://drive.google.com/file/d/1m4-oHYZYalk4VKcs_GDh-9UB657PcMSw/view?usp=sharing"
    url_ca = "https://drive.google.com/file/d/1RPoTtj4RWtx8QsJ9RokFi5r6Wd4y805v/view?usp=sharing"
    if not os.path.exists('fixnet'):
        wget.download(url_fix, 'fixnet')
    if not os.path.exists('casme'):
        wget.download(url_ca, 'casme')
    
    path = 'casme'
    
    model = casme_load_model(path)
    print(model)
